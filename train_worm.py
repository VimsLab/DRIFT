...
from worm_model import Encoder, DecoderWithAttention
...
data_name = 'Demo_worm'  # base name shared by data files
save_weights_name = cfg.save_weights_name

data_folder_p = cfg.data_folder_p # base name shared by data files
jason_file_p = cfg.jason_file_p # base name shared by data files

binary_folder = cfg.binary_folder
...
# Model parameters
emb_dim = 2  # dimension of word embeddings
attention_dim = 512  # dimension of attention linear layers _ NOT USED FOR Final version FILAMENTS
decoder_dim = 512  # dimension of decoder RNN
dropout = 0.5
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # sets device for model and PyTorch tensors
# device = torch.device("cuda:1,3" if torch.cuda.is_available() else "cpu")  # sets device for model and PyTorch tensors
...
best_bleu4 = 30000. # BLEU-4 score right now
...
def main():
    ...
    # criterionShort = short_offset_loss().to(device)
    ...
    train_loader_p = torch.utils.data.DataLoader(
        worm_COCO(data_folder_p, jason_file_p, image_folder, binary_folder, transforms.Compose([transforms.ToTensor(), normalize]), True),
    ...
    val_loader_p = torch.utils.data.DataLoader(
        worm_COCO(data_folder_val, jason_file_val, image_folder_val, binary_folder, transforms.Compose([transforms.ToTensor(), normalize]), True),
        batch_size=batch_size, shuffle=True, num_workers=workers, pin_memory=True, drop_last=True)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        recent_bleu4 = validate(val_loader=val_loader_p,
                                encoder=encoder,
                                decoder=decoder,
                                decoderMulti=decoderMulti,
                                criterion=criterion)
        ...
        best_bleu4 = min(recent_bleu4, best_bleu4)
        if not is_best:
        ...
        # best_bleu4 = None
        # Save checkpoint
            save_checkpoint(data_name+save_weights_name, epoch, epochs_since_improvement, encoder, decoder, encoder_optimizer,
                        decoder_optimizer, decoderMulti, decoderMulti_optimizer, best_bleu4, is_best)
...
def train(train_loader, encoder, decoder, decoderMulti, criterion, encoder_optimizer, decoder_optimizer, decoderMulti_optimizer, epoch):
    ...
    loss_semantic_q = AverageMeter()  

    start = time.time()
    # Batches
    for i, (imgs, img_org, sequence, sequence_offset, sequence_len, stop_sequence, ground_truth_multi, image_level_gt, meta) in enumerate(train_loader):
        '''
        input -> imgs
              -> patch_sequece
        Stage 1: 
        ...
        loss_semantic = criterion[0](mask_pred, image_level_gt[:,2:3,:,:])
        ...
        scores, predictionsStop, caps_sorted, decode_lengths, offset_prediction, segmentation_prediction, alphas, sort_ind = decoder(imgs_encoded, img_org, sequence, sequence_offset, sequence_len)
        ...
        loss = lossbi +  2*losso_segmentation + 1*loss_keypoints + 2*lossoffset_mid + lossoffset + loss_semantic
        ...
        loss_semantic_q.update(loss_semantic.item(), batch_size)
        ...
        if i % print_freq == 0:
            ...
            print('Epoch: [{0}][{1}/{2}]\t'
                    ...
                    'loss_semantic {loss_semantic.val:.4f} ({loss_semantic.avg:.4f})\t'
...
def validate(val_loader, encoder, decoder, decoderMulti, criterion):
    ...
    with torch.no_grad():
        # Batches
        for i, (imgs, img_org, sequence, sequence_offset, sequence_len, stop_sequence, ground_truth_multi, image_level_gt, meta) in enumerate(val_loader):
           ...
           # Forward prop.
            ...
            loss = lossbi +  losso_segmentation + loss_keypoints + lossoffset_mid + lossoffset + 1 * loss_semantic
