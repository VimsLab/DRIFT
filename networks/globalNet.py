...
class globalNet(nn.Module):
    def __init__(self, channel_settings, output_shape, num_class):
        ...
        for i in range(len(channel_settings)):
            ...
            predict.append(self._predict(output_shape, 2))
            predict_offset.append(self._predict(output_shape, num_class))
    ...
    def forward(self, x):
        global_fms, global_outs, global_offset = [], [], []
        for i in range(len(self.channel_settings)):
            ...
            global_offset.append(self.predict_offset[i](feature))
            global_outs.append(torch.sigmoid(feature_tmp))
            # if i != len(self.channel_settings) - 1:
            #     global_offset = self.upsamples[i](feature)
        x = feature

        return x, global_fms, global_outs,global_offset
